...
function Form({ itemVariants }) {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(
        "service_1s7wjjh", // EmailJS service ID
        "template_zzy5e38", // EmailJS template ID
        form.current,
        {
          publicKey: "mAyHJxeSolHxUesnC", // EmailJS publicKey
        }
      )
      .then(
        () => {
        ...
        (error) => {
          setStatus(error.text || "Failed to send");
          setIsSubmitting(false);
  ...
  const StatusMessage = () => {
    ...
    const config = {
      ...
      error: {
        icon: "✕",
        message: `Failed: ${typeof status === "string" && status !== "error"
          ? status
          : "Please try again."
          }`,
        bgColor: "bg-red-100",
    ...
    const { icon, message, bgColor, textColor, borderColor } = status === 'success' ? config.success : config.error;
